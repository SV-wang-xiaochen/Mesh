...
count_intersected = np.zeros(Z_LENS_MAX-Z_LENS_MIN+1)
...
for mesh_nr in range(0, len(obj_list)):
    ...
    intersected = np.zeros(Z_LENS_MAX - Z_LENS_MIN + 1)
    for Z_LENS in range(Z_LENS_MIN, Z_LENS_MAX+1):
        ...
        # print(len(intersection_segments_within_circle))
        if len(intersection_segments_within_circle)>0:
            intersected[Z_LENS-Z_LENS_MIN] = 1
        # print(f"Z_LENS:{Z_LENS}, intersected: {intersected}")
    count_intersected += intersected
    intersected = np.zeros(Z_LENS_MAX - Z_LENS_MIN + 1)

# Create the plot
lens_distance = np.linspace(3, 30, 28)

plt.figure(figsize=(12, 6))  # Adjust the values as needed
plt.bar(lens_distance, count_intersected, width=0.5, align='center', alpha=0.7, color='b')

# Add labels on top of each bar
for i in range(len(lens_distance)):
    plt.text(lens_distance[i], count_intersected[i], str(int(count_intersected[i])), ha='center', va='bottom', color= 'r' if count_intersected[i]>=3 else 'g')

# Set x-axis ticks to match the x values
plt.xticks(lens_distance, [str(int(val)) for val in lens_distance])

# Add labels and title
plt.xlabel('Lens Distance: mm')
plt.ylabel('Number of Head Hit')
plt.title(f'Lens Radius: {RADIUS} mm')
